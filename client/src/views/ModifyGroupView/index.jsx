...
import { getGroupById, removeExerciseFromGroup } from "../../proxies";
import './style.css';
...
const ModifyGroupView = () => {
  ...
  if (loading) return <div className="modify-group__loading">Loading group...</div>;
  if (!group) return <div className="modify-group__error">Group not found</div>;

  return (
    <div className="modify-group">
      <h2 className="modify-group__title">Modify Group: {group.groupName}</h2>
      {group.exercises.length === 0 && <p className="modify-group__empty">No exercises in this group.</p>}
      <ul className="modify-group__list">
        {group.exercises.map((exercise) => (
          <li key={exercise.id} className="modify-group__item">
            ...
            <button
              className={`modify-group__remove-btn ${removing[exercise.id] ? 'modify-group__remove-btn--disabled' : ''}`}
