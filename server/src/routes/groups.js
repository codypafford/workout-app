...
router.get('/', async (req, res) => {
  try {
    ...
    const lastLogsAggregate = await Log.aggregate([
      ...
      { $sort: { date: -1 } }, // newest first
      {
        $group: {
          ...
          logs: { $push: { sets: '$sets', reps: '$reps', weight: '$weight', date: '$date' } },
    ...
    lastLogsAggregate.forEach(entry => {
      lastLogsMap.set(`${entry._id.groupId}-${entry._id.exerciseId}`, entry.logs.filter(
        log => new Date(log.date).setHours(0,0,0,0) === new Date(entry.mostRecentDate).setHours(0,0,0,0)
      ));
    });
    ...
    const transformed = groups.map(group => ({
      ...
      exercises: group.exerciseIds.map(exercise => {
        ...
        lastLogsForExerciseRaw.forEach(log => {
          ...
          combinedLastLogsMap.set(key, (combinedLastLogsMap.get(key) || 0) + log.sets);
        });
        const lastLogsForExercise = Array.from(combinedLastLogsMap.entries()).map(([key, sets]) => {
