...
const AddWorkoutModal = ({ show, onClose, onAdd }) => {
  ...
  const handleAddClick = () => {
    if (selectedWorkoutId === 'new' && newWorkoutName.trim()) {
      onAdd({ id: Date.now(), name: newWorkoutName });
      onClose();
    } else {
      const selectedWorkout = MOCK_SAVED_WORKOUTS.find(
        (w) => w.id === parseInt(selectedWorkoutId)
      );
      if (selectedWorkout) {
  ...
  return (
    <div className="modal-backdrop">
      <div className="modal-content">
        ...
        <label htmlFor="workout-select">Select a workout or add new:</label>
        <select
          id="workout-select"
